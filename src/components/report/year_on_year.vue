  ...
  export default {
    ...
    methods: {
      ...
      drawLine(){
        ...
        // 绘制图表
        barChart.setOption({
          title: {text: 'Year On Year Last 3 Years'},
          legend: {
            data:this.last3YearsOnYear.legendData,
            top:30,
          },
          toolbox: {
            x:'right',
          ...
          xAxis: {
            type: 'category',
            data: this.last3YearsOnYear.xAxisData,
          },
          series: this.last3YearsOnYear.seriesData
        });
    ...
    mounted() {
      this.$store.dispatch("dashBord/getLast3YearOnYearData").then(()=>{
        for(var i=0; i<this.last3YearsOnYear.xAxisData.length; i++){
          var subPieData = [];
          for(var j=0; j<this.last3YearsOnYear.seriesData.length; j++){
            subPieData.push({
              value: this.last3YearsOnYear.seriesData[j].data[i],
              name: this.last3YearsOnYear.seriesData[j].name
            });
          }
          this.pieData.push(
            {
              title:this.last3YearsOnYear.xAxisData[i],
              data:subPieData
            }
          );
    ...
    computed: {
      ...mapState({
        last3YearsOnYear: state => state.dashBord.last3YearsOnYear
      }),
    }
  }
</script>
