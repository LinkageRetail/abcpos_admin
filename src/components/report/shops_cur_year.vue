  ...
  export default {
    ...
    methods: {
      ...
      drawLine(){
        ...
        // 绘制图表
        myChart.setOption({
          title: {text: 'Current Year To Now'},
          tooltip: {},
          toolbox: {
            x:'right',
            y:'top',
          ...
          xAxis: {
            type: 'category',
            data: this.curYearToNowByShop.shops,
          },
          series: [{
            data: this.curYearToNowByShop.amts,
            name: 'Sales Amount',
            type: 'bar',
            markLine : {
              data: [
    ...
    mounted() {
      this.$store.dispatch("dashBord/getYearToNowByShop").then(()=>{
        for(var i=0; i<this.curYearToNowByShop.shops.length; i++){
          this.pieData.push({
            value: this.curYearToNowByShop.amts[i],
            name: this.curYearToNowByShop.shops[i]
          });
        }
        this.drawLine();
        this.drawPie();
    ...
    computed: {
      ...mapState({
        curYearToNowByShop: state => state.dashBord.curYearToNowByShop,
      }),
    }
  }
</script>
