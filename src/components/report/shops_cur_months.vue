...
import { mapState} from "vuex";
  ...
  export default {
    ...
    methods: {
      ...
      drawLine() {
        ...
        barChart.setOption({
          ...
          xAxis: {
            type: 'category',
            data: this.curMonthToNowByShop.shops,
          },
          series: [{
            data: this.curMonthToNowByShop.amts,
            name: 'Sales Amount',
            type: 'bar',
            markLine: {
              data: [
    ...
    mounted() {
      this.$store.dispatch("dashBord/getMonthToNowByShop").then(()=>{
        for(var i=0; i<this.curMonthToNowByShop.shops.length; i++){
          this.pieData.push({
            value: this.curMonthToNowByShop.amts[i],
            name: this.curMonthToNowByShop.shops[i]
          });
        }
        this.drawLine();
        this.drawPie();
    ...
    computed: {
      ...mapState({
        curMonthToNowByShop: state => state.dashBord.curMonthToNowByShop,
      }),
    }
  }
</script>
