<template>
  <q-page>
    ...
    <div id="pie_chart"  class="right_pie float_left ">
      ...
      <div id="pie_chart6" class="height_400 width_100_p float_left "></div>
    </div>
    <div
  ...
  export default {
    ...
    methods: {
      ...
      drawLine(){
        ...
        // 绘制图表
        barChart.setOption({
          title: {text: 'MoM in the past 7 Days'},
          tooltip: {
            trigger: 'axis'
          },
          grid: {
          ...
          legend: {
            ...
            x:'center',
            y:'top',
            data:this.last7DaysData.legendData
          },
          toolbox: {
            x:'right',
            y:'top',
          ...
          xAxis: {
            data: this.last7DaysData.xAxisData,
          },
          series: this.last7DaysData.seriesData
        });
    ...
    mounted() {
      this.$store.dispatch("dashBord/getLast7DaysSalesData").then(()=>{
        for(var i=0; i<this.last7DaysData.xAxisData.length; i++){
          var subPieData = [];
          for(var j=0; j<this.last7DaysData.seriesData.length; j++){
            subPieData.push({
              value: this.last7DaysData.seriesData[j].data[i],
              name: this.last7DaysData.seriesData[j].name
            });
          }
          this.pieData.push(
            {
              title:this.last7DaysData.xAxisData[i],
              data:subPieData
            }
          );
    ...
    computed: {
      ...mapState({
        last7DaysData: state => state.dashBord.last7DaysData,
      }),
    }
  }
</script>
